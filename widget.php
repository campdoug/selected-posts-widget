...
class Selected_Posts_Widget_Widget extends WP_Widget {
	function __construct() {
		...
		parent::__construct( false, __('Selected Posts Widget', 'chef' ) );
	}

	function widget( $args, $instance ) {
		include $this->path . '/widget-template.php';
	}
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Featured Posts', 'chef' );
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts' ) ); ?>"><?php _e( esc_attr( 'Select Post', 'chef' ) ); ?></label>
			<div class="post-suggestion-wrap">
				<input class="widefat post-suggestion" id="<?php echo esc_attr( $this->get_field_id( 'posts' ) ); ?>" type="text" placeholder="<?php _e('Slowly type for suggestions', 'chef'); ?>" autocomplete="off" />
